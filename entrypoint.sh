...
sed -i 's/localhost/127.0.0.1/g' ~/.pipscoin/mainnet/config/config.yaml
...
if [[ ${testnet} == "true" ]]; then
  if [[ -z $full_node_port || $full_node_port == "null" ]]; then
    pipscoin configure --set-fullnode-port 58444
  else
    pipscoin configure --set-fullnode-port ${var.full_node_port}
  fi
fi
